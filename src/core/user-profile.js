...
export class KLCUserProfileManager {
  constructor() {
    ...
    this.DEFAULT_PROFILE = {
      name: 'Pegawai',
      userType: 'Pegawai Kementerian Keuangan',
      role: 'Pegawai Kementerian Keuangan',
      avatarUrl: '',
      nip: '-',
      email: '-',
      unit: 'Kementerian Keuangan RI',
      organisasi: 'Kementerian Keuangan RI'
    };

    this.fallbackProfile = { ...this.DEFAULT_PROFILE };
  ...
  onProfileChange(fn) {
    if (typeof fn === 'function') {
      ...
      if (this.currentUser) {
        try { fn(this.getProfile()); } catch(e){}
      }
    }
    return () => {
  ...
  async init() {
    ...
    if (savedProfile && savedProfile.name && savedProfile.name !== 'Peserta' && savedProfile.name !== 'Pegawai DJBC') {
      ...
      this._authMethod = 'LOCAL_STORAGE';
    ...
    if (typeof window !== 'undefined' && window.location && typeof URLSearchParams !== 'undefined') {
      ...
      if (urlParams.has('username') || urlParams.has('nip') || urlParams.has('name')) {
        const urlUser = {
          name: urlParams.get('name') || urlParams.get('username') || 'Pegawai DJBC',
          avatarUrl: urlParams.get('avatar') || urlParams.get('picture') || '',
          userType: (urlParams.get('role') || 'PEGAWAI DJBC').toUpperCase(),
          role: (urlParams.get('role') || 'PEGAWAI DJBC').toUpperCase(),
        ...
        this.saveProfileToStorage(urlUser);
        this.render(urlUser);
      }
    }
  ...
  getProfile() {
    return this.currentUser || this.fallbackProfile;
  }
  ...
  getActorForXAPI() {
    ...
    const cleanNip = p.nip && p.nip !== '-' ? p.nip : 'pegawai';
    const emailStr = (p.email && p.email !== '-' && p.email.includes('@'))
      ? p.email
      : `${cleanNip}@kemenkeu.go.id`;

    return {
      objectType: 'Agent',
      name: p.name || 'Pegawai DJBC',
      mbox: `mailto:${emailStr}`,
      account: {
        homePage: 'https://klc2.kemenkeu.go.id',
        name: p.nip && p.nip !== '-' ? p.nip : (p.username || p.name)
      }
    };
  }
      ...
      if (typeof window !== 'undefined' && window.top && window.top !== window) {
        ...
        if (topUser) {
          ...
          if (parsed && parsed.name && parsed.name !== 'Peserta' && parsed.name !== 'Pegawai DJBC') {
            ...
            this._authMethod = 'PARENT_WINDOW';
  ...
  checkScormUserData() {
    try {
      if (typeof window !== 'undefined') {
        ...
        if (window.scorm && typeof window.scorm.getValue === 'function') {
          scormName = window.scorm.getValue('cmi.core.student_name');
          scormId = window.scorm.getValue('cmi.core.student_id') || '-';
        } else if (window.scormInstance) {
          scormName = window.scormInstance.learnerName;
          scormId = window.scormInstance.learnerId;
        }
   ...
   * Memproses name, nip, email, dan picture (avatarUrl)
   */
  parseProfileData(json) {
    ...
    let userPhoto = userObj.picture || userObj.image || userObj.image_url || userObj.avatar || userObj.avatarUrl || userObj.avatar_url || userObj.photo || json.picture || json.image || json.avatar || '';
    ...
    const nip = userObj.nip || userObj.preferred_username || userObj.username || json.nip || '-';
    const email = userObj.email || json.email || (nip !== '-' ? `${nip}@kemenkeu.go.id` : '-');
    ...
    return {
      ...
      email: email,
      nip: nip,
      unit: userObj.unit || userObj.unitKerja || 'Direktorat Jenderal Bea dan Cukai',
      organisasi: userObj.organisasi || 'Kementerian Keuangan RI'
    };
  }
   ...
   * - user-display-role -> nip (atau format NIP. XXXXXX)
   * - user-avatar-container -> picture (avatarUrl)
   */
  render(profile) {
    if (typeof document === 'undefined') return;
    ...
    nameEls.forEach(el => { el.textContent = user.name || 'Pegawai'; });

    // Tempatkan NIP pada user-display-role sesuai petunjuk
    const roleText = (user.nip && user.nip !== '-')
      ? `NIP. ${user.nip}`
      : (user.userType || user.role || 'Pegawai Kementerian Keuangan');
    roleEls.forEach(el => { el.textContent = roleText; });

    initialsEls.forEach(el => { el.textContent = initials; });

    const avatarPhoto = user.avatarUrl || user.picture;
    ...
    if (avatarPhoto) {
      imgEls.forEach(img => {
        ...
        img.onerror = () => {
          ...
          placeholderEls.forEach(p => { p.style.display = 'flex'; });
          initialsEls.forEach(i => { i.style.display = 'flex'; });
        };
      });
  ...
  /**
   * Menampilkan Modal Detail Profil User KLC2
   */
  showProfileDetailModal(profile) {
    ...
    if (!modal) {
      ...
      modal.style.cssText = 'position:fixed; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.65); backdrop-filter:blur(6px); z-index:999999; padding:16px;';
      document.body.appendChild(modal);
    }

    const user = profile || this.getProfile() || this.fallbackProfile;
    const initialText = (user.name || 'BC').trim().split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'BC';
    const avatarPhoto = user.avatarUrl || user.picture;
      ...
      <div style="background:linear-gradient(145deg, #0B3A6F 0%, #062347 100%); border:1px solid #D9B45B; border-radius:18px; padding:24px; max-width:360px; width:100%; color:#FFFFFF; box-shadow:0 20px 40px rgba(0,0,0,0.4); position:relative; font-family:'Poppins',sans-serif;">
        ...
        <div style="display:flex; flex-direction:column; align-items:center; text-align:center;">
          ...
          <div style="width:76px; height:76px; border-radius:50%; border:2.5px solid #D9B45B; overflow:hidden; margin-bottom:12px; box-shadow:0 4px 14px rgba(0,0,0,0.3); background:#0B3A6F; display:flex; align-items:center; justify-content:center;">
            ${avatarPhoto ? `<img src="${avatarPhoto}" style="width:100%; height:100%; object-fit:cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">` : ''}
            <span style="font-size:22px; font-weight:700; color:#D9B45B; ${avatarPhoto ? 'display:none;' : ''}">${initialText}</span>
          </div>
